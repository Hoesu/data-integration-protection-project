...
식별자 노출위험은 특정 노드가 전체 네트워크 내에서 구조적 혹은 통계적으로 고립된 형태를 보일 때 발생할 것으로 예상한다.  
단순히 가중치 합 $S_i$만 사용할 경우, 연결 2개(각 1.0)와 연결 20개(각 0.1)가 동일한 $S_i$를 가질 수 있어  
연결 수 차이가 반영되지 않는 문제가 있다.

이를 보완하기 위해 **평균 가중치 기반** 정의를 사용한다. 노드 $i$에 연결된 모든 엣지의 가중치 합을 $S_i$,  
이웃 수를 $k_i$라고 할 때, 평균 가중치는 $S_i / k_i$로 정의된다.

$$
IDR_i =
\begin{cases}
1, & k_i = 0 \\\\
\frac{1}{1 + S_i / k_i}, & k_i > 0
\end{cases}
$$

연결이 없으면($k_i = 0$) 완전히 고립된 노드로 보고 최대 위험(1)에 가깝게 설정하며,  
연결이 많지만 각 엣지가 약한 노드는 평균 가중치가 작아 상대적으로 높은 위험도로 평가된다.
...
$$
ADR_i = \frac{1}{S_i(k_i-1)} \sum_{j \neq i} \sum_{k \neq i,j} \frac{w_{ij} + w_{ik}}{2} a_{jk}
$$
...
마지막으로, 두 위험 차원은 서로 상반된 방향성을 지니므로, 개별 관측치의 최종 노출위험 척도는  
IDR과 ADR을 정규화하여 결합한 형태로 정의한다. 노드 $i$에 대한 최대 노출 위험을 단순히 두 값의 최대값으로
정의할 수도 있지만, 본 구현에서는 두 위험을 모두 반영하기 위해 정규화 후 합산하는 방식을 기본으로 사용한다.

정규화 함수 $\text{norm}(\cdot)$를 사용하면, 기본 MDR 정의는 다음과 같다.

$$
MDR_i = \text{norm}(IDR_i) + \text{norm}(ADR_i)
$$

여기서 정규화는 각 위험도의 최소–최대 범위를 0–1로 스케일링하는 Min-Max 정규화를 사용한다.  
이를 통해 두 위험 차원의 스케일 차이를 제거하고, 두 위험을 모두 반영하는 최종 위험도를 얻는다.  
데이터셋 단위 노출 위험은 개별 관측치 단위 노출 위험도의 상위 $n\%$의 평균값으로 측정한다.
